<script lang="ts">
  ...
  async function load() {
    ...
    const { data } = await rq.apiEndPoints().GET('/api/v1/posts/byAuthor/{authorId}', {
      params: {
        path: { authorId: parseInt($page.params.authorId) },
        query: {
          kw,
          kwType,
          page: page_
...
<div class="flex-grow flex justify-center">
  <div class="w-full max-w-screen-2xl px-2 my-4">
    <div class="text-center">
      <div class="font-bold text-lg">{$page.params.authorId}번 개발자님의 공개글</div>
      <div class="mt-3 text-gray-400">
        {$page.params.authorId}번 개발자님이 작성하신 공개글 입니다.
      </div>
    </div>
